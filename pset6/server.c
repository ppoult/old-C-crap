...
void stop(void);
int no_of_char(char* stack, char needle);
...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            if (no_of_char(request, ' ') != 2) {
                error(404);
                continue;
            }
            // end with \r\n
            if (strcmp(strstr(request, "\r\n"), "\r\n") != 0) {
                error(404);
                continue;
            }
            ...
            method = strncpy(method, request, sizeof(char) * 3);
            // method not GET, return 405
            if (strcmp(method, "GET") != 0) {
                error(405);
                continue;
            }
            // request-target = absolute-part["?" query]
            char* request_wo_method = strchr(request, ' ');
            request_wo_method++;
            char* http = strchr(request_wo_method, ' ');
            http++;

            int request_target_len = strlen(request_wo_method) - strlen(http);
            char* request_target = malloc(sizeof(char) * request_target_len);
            strncpy(request_target, request_wo_method, sizeof(char) * request_target_len);
            // printf("request_target: %s\n", request_target);
            // no more than one ?
            if (no_of_char(request_target, '?') > 1) {
                error(400);
                continue;
            }
            // not start with "/", return 501
            if (request_target[0] != '/') {
                error(501);
                continue;
            }
            ...
            // does not contain . and  file extension, return 501
            if (!strchr(request_target, '.')) {
                error(501);
                continue;
            }

            // HTTP-version not being v1.1, return 505
            ...
            // get absolute-path
            int len_absolutepath = strlen(request_target) - strlen(query);
            if (strchr(request_target, '?')) {
                len_absolutepath -= 1;
            }
            char* absolute_path = malloc(sizeof(char) * len_absolutepath + 1);
            strncpy(absolute_path, request_target, len_absolutepath);
            // concat with root
            int len_path = strlen(root) + strlen(absolute_path) + 1;
...
const char* lookup(const char* extension)
{
    // convert extension to lower string
    char* lower_extension = malloc(sizeof(char) * strlen(extension) + 1);
    int last = 0;
    for (int i = 0; i < strlen(extension); ++i) {
        lower_extension[i] += tolower(extension[i]);
        last = i;
    }
    lower_extension[last + 1] = '\0';

    // define value returned
    if (strcmp(lower_extension, "css") == 0)
        return "text/css";
    if (strcmp(lower_extension, "html") == 0)
        return "text/html";
    if (strcmp(lower_extension, "gif") == 0)
        return "image/gif";
    if (strcmp(lower_extension, "ico") == 0)
        return "image/x-icon";
    if (strcmp(lower_extension, "jpg") == 0)
        return "image/jpeg";
    if (strcmp(lower_extension, "js") == 0)
        return "text/javascript";
    if (strcmp(lower_extension, "png") == 0)
        return "image/png";
...
/**
 * Helper function to find recurrence of character in string
 */
int no_of_char(char* stack, char needle) {
    // finding recurrence of needle in stack
    int count = 0;
    ...
    char* next_stack = strchr(stack, needle);
    if (!next_stack) {
        return 0;
    } else {
        count = 1;
    ...
    count += no_of_char(++next_stack, needle);
    return count;
}
